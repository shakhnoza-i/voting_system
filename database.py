...
def create_poll(connection, title: str, owner: str, options: List[str]):
    with connection:
        with connection.cursor() as cursor:
            ...
            execute_values(cursor, INSERT_OPTION, option_values) # cursor to execute all of the different queries
            # for option_value in option_values:  # replased by execute_values
